...
static UINT32 transport_krb5_check_account(rdpTransport *transport,
  ...
  if ((ret = krb5_init_context(&context)) != 0)
    WLog_Print(transport->log, WLOG_ERROR,
               "krb5_init_context failed with error %d", (int)ret);
  else if ((ret = krb5_parse_name_flags(context, address, 0, &principal)) != 0)
    WLog_Print(transport->log, WLOG_ERROR,
               "krb5_parse_name_flags failed with error %d", (int)ret);
  /* Find a credential cache with a specified client principal */
    ...
    if ((ret = krb5_cc_default(context, &ccache)) != 0)
      WLog_Print(transport->log, WLOG_ERROR,
                 "krb5 failed to resolve credentials cache with error %d",
                 (int)ret);
  }
  ...
  else if ((ret = krb5_init_creds_init(context, principal, NULL, NULL, 0, NULL,
                                       &ctx)) != 0) {
    WLog_Print(transport->log, WLOG_WARN,
               "krb5_init_creds_init returned error %d", (int)ret);
    goto out;
  }
  /* Set a password for acquiring initial credentials */
  else if ((ret = krb5_init_creds_set_password(context, ctx, passwd)) != 0) {
    WLog_Print(transport->log, WLOG_WARN,
               "krb5_init_creds_set_password returned error %d", ret);
    goto out;
  }
...
out:
  ...
  case KRB5_KDC_UNREACH:
    WLog_Print(transport->log, WLOG_WARN,
               "krb5_init_creds_get: KDC unreachable");
  ...
  case KRB5_GET_IN_TKT_LOOP:
    WLog_Print(transport->log, WLOG_WARN,
               "krb5_init_creds_get: Password incorrect");
    ret = FREERDP_ERROR_AUTHENTICATION_FAILED;
    break;

  case KRB5KDC_ERR_KEY_EXP:
    WLog_Print(transport->log, WLOG_WARN,
               "krb5_init_creds_get: Password has expired");
    ret = FREERDP_ERROR_CONNECT_PASSWORD_EXPIRED;
    break;

  case KRB5KDC_ERR_CLIENT_REVOKED:
    WLog_Print(transport->log, WLOG_WARN,
               "krb5_init_creds_get: Password revoked");
  ...
  if (where & SSL_CB_ALERT) {
    ...
    case (SSL3_AL_FATAL << 8) | SSL_AD_ACCESS_DENIED: {
      if (!freerdp_get_last_error(transport->context)) {
        WLog_Print(transport->log, WLOG_ERROR, "%s: ACCESS DENIED",
                   __FUNCTION__);
      ...
      if (transport->NlaMode) {
        ...
        if ((transport->settings->Domain != NULL) &&
          ...
          kret = transport_krb5_check_account(
              transport, transport->settings->Username,
...
BOOL transport_connect_nla(rdpTransport *transport) {
  ...
  if (settings->AuthenticationServiceClass) {
    if (!nla_set_service_principal(
            rdp->nla, (char *)nla_make_spn(settings->AuthenticationServiceClass,
...
BOOL transport_connect(rdpTransport *transport, const char *hostname,
  ...
  if (transport->GatewayEnabled) {
    if (!status && settings->GatewayHttpTransport) {
      ...
      if (status) {
        transport->frontBio =
            rdg_get_front_bio_and_take_ownership(transport->rdg);
    ...
    if (isProxyConnection)
      sockfd = freerdp_tcp_connect(context, settings, proxyHostname, peerPort,
                                   timeout);
    else
    ...
    if (isProxyConnection) {
      if (!proxy_connect(settings, transport->frontBio, proxyUsername,
                         proxyPassword, hostname, port))
        return FALSE;
    }
...
#define WLog_ERR_BIO(transport, biofunc, bio)                                  \
  transport_bio_error_log(transport, biofunc, bio, __FILE__, __FUNCTION__,     \
                          __LINE__)

static void transport_bio_error_log(rdpTransport *transport, LPCSTR biofunc,
                                    BIO *bio, LPCSTR file, LPCSTR func,
                                    DWORD line) {
  ...
  if (ERR_peek_error() == 0) {
    ...
    WLog_PrintMessage(transport->log, WLOG_MESSAGE_TEXT, level, line, file,
                      func, fmt, biofunc, saveerrno, strerror(saveerrno));
    return;
  }
...
static SSIZE_T transport_read_layer(rdpTransport *transport, BYTE *data,
                                    size_t bytes) {
  SSIZE_T read = 0;
 ...
 * In case it was not possible to read toRead bytes 0 is returned. The stream is
 * always advanced by the number of bytes read.
 *
 ...
 * @return < 0 on error; 0 if not enough data is available (non blocking mode);
 * 1 toRead bytes read
 */
...
/**
 * @brief Try to read a complete PDU (NLA, fast-path or tpkt) from the
 * underlying transport.
 *
 * If possible a complete PDU is read, in case of non blocking transport this
 * might not succeed. Except in case of an error the passed stream will point to
 * the last byte read (correct position). When the pdu read is completed the
 * stream is sealed and the pointer set to 0
 *
 * @param[in] transport rdpTransport
 * @param[in] s wStream
 * @return < 0 on error; 0 if not enough data is available (non blocking mode);
 * > 0 number of bytes of the *complete* pdu read
 */
int transport_read_pdu(rdpTransport *transport, wStream *s) {
  ...
  if (transport->NlaMode) {
    ...
    if (header[0] == 0x30) {
      ...
      if (header[1] & 0x80) {
        if ((header[1] & ~(0x80)) == 1) {
          ...
          if (position < 3 && (status = transport_read_layer_bytes(
                                   transport, s, 3 - position)) != 1)
            return status;
          ...
          /* check for header bytes already was readed in previous calls */
          if (position < 4 && (status = transport_read_layer_bytes(
                                   transport, s, 4 - position)) != 1)
            return status;
    ...
    if (header[0] == 0x03) {
      ...
      /*
       * fast-path has 7 bits for length so the maximum size, including headers
       * is 0x8000 The theoretical minimum fast-path PDU consists only of two
       * header bytes plus one byte for data (e.g. fast-path input synchronize
       * pdu)
       */
      if (pduLength < 3 || pduLength > 0x8000) {
        WLog_Print(transport->log, WLOG_ERROR,
                   "fast path - invalid pduLength: %" PRIdz, pduLength);
        return -1;
      }
    }
  }
...
int transport_write(rdpTransport *transport, wStream *s) {
  ...
  while (length > 0) {
    ...
    if (status <= 0) {
      /* the buffered BIO that is at the end of the chain always says OK for
       * writing, so a retry means that for any reason we need to read. The most
       * probable is a SSL or TSG BIO in the chain.
       */
...
DWORD transport_get_event_handles(rdpTransport *transport, HANDLE *events,
  ...
  if (events) {
    if (count < 1) {
      WLog_Print(transport->log, WLOG_ERROR,
                 "%s: provided handles array is too small", __FUNCTION__);
      return 0;
    }
  ...
  if (!transport->GatewayEnabled) {
    ...
    if (events) {
      ...
      if (BIO_get_event(transport->frontBio, &events[1]) != 1) {
        WLog_Print(transport->log, WLOG_ERROR,
                   "%s: error getting the frontBio handle", __FUNCTION__);
        return 0;
      }
    }
  } else {
...
int transport_check_fds(rdpTransport *transport) {
  ...
  while (now < dueDate) {
     ...
     */
    if ((status = transport_read_pdu(transport, transport->ReceiveBuffer)) <=
        0) {
      if (status < 0)
        WLog_Print(transport->log, WLOG_DEBUG,
    ...
    if (!(transport->ReceiveBuffer =
              StreamPool_Take(transport->ReceivePool, 0)))
      return -1;
    ...
    if (recv_status < 0) {
      WLog_Print(transport->log, WLOG_ERROR,
                 "transport_check_fds: transport->ReceiveCallback() - %i",
                 recv_status);
      return -1;
    }
